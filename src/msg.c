...
int parse_msg_data_req(const unsigned char *msg, int size, SMsgDataReq *req)
{
	...
	req->minute = msg[8];
	req->second = msg[8];
	req->gps_info_len = (unsigned char)(msg[9] >> 4 & 0x0f);
	req->gps_pos_num = (unsigned char)(msg[9] & 0x0f);
	req->latitude = (unsigned int)(msg[10] << 24 | msg[11] << 16 | msg[12] << 8 | msg[13]);
	req->longitude = (unsigned int)(msg[14] << 24 | msg[15] << 16 | msg[16] << 8 | msg[17]);
	req->speed = msg[18];
	req->rp_dp_flag = (unsigned char)(msg[19] >> 5 & 0x01);
	req->positioned_flag = (unsigned char)(msg[19] >> 4 & 0x01);
	req->latitude_type = (unsigned char)(msg[19] >> 3 & 0x01);
	req->longitude_type = (unsigned char)(msg[19] >> 2 & 0x01);
	req->heading = (unsigned short)((msg[19] & 0x03) << 8 | msg[20]);
	req->mcc = (unsigned short)(msg[21] << 8 | msg[22]);
	req->mnc = (unsigned short)(msg[23] << 8 | msg[24]);
